...
public class BookDAO {
	...
	public int bookInsert(BookBean bdata) {
		...
		try {
			...
			sql = "insert into Book(BOOK_ID, CONCERT_ID, MEMBER_ID, BOOK_ETICKET, BOOK_AMOUNT, CARD_ID, BOOK_DATE, BOOK_PAYMENT) "
					+ "values(?, ?, ?, ?, ?, ?, sysdate, ?)";
	...
	public ConcertBeanJoin getReserve(int concert_id, int num) {
		...
		try {
			...
			String sql = "select * from book, CONCERT " + "where book.concert_id = concert.concert_id "
					+ "and book.concert_id = ? and book.book_id=?";
